      ...
      <div class="tenementContent">
        <el-form label-width="150px" :label-position="left">
          <el-form-item label="所属小区" class="formItem">
            ...
            <el-select v-model="selectData.communityId" filterable placeholder="请选择'所属小区"  @change="handleSelect" class="tenementForm">
              <el-option
          ...
          <el-form-item class="formItem" >
            <el-button type="primary" class="save" @click="editBuildingList">保存</el-button>
...
<script>
import { updataBuilding, get_building } from 'API/buildingList'
  ...
  methods: {
    ...
    queryBuildingInfo() {
      ...
      get_building({
        ...
        let data = res.data
        this.selectData.communityId = data.communityId
        this.itemData.forEach((element, index, arr)=>{
          if(arr[index].key =='name') {
            arr[index].value = data.name
          } else if(arr[index].key == 'floor') {
            arr[index].value = data.floor
          } else if(arr[index].key == 'unit') {
            arr[index].value = data.unit
          } else if(arr[index].key == 'type') {
            arr[index].value = data.type
          } else if(arr[index].key == 'struct') {
            arr[index].value == data.struct
          } else if(arr[index].key == 'orientation') {
            arr[index].value = data.orientation
          } else if(arr[index].key == 'remark') {
            arr[index].value = data.remark
          }
        })
      })
    },
